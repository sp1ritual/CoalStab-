...
class CoalStabApp(tk.Tk):
    ...
    def show_sensor_list(self):
        ...
        sensor_date = ttk.Entry(sensor_form_frame)
    ...
    def show_dashboard(self):
        ...
        ax1.set_title('Динамическая стабильность')
        ax1.set_xlabel('')
        ax1.set_ylabel('Динамическое сопротивление')
        ax1.legend(['Динамическая стабильность'], loc='upper right')
    ...
    def show_feedback_form(self):
        ...
        ttk.Label(
            feedback_form_frame, text="ФИО:", background="white"
        ).grid(row=0, column=0, pady=5, sticky="e")
        feedback_name = ttk.Entry(feedback_form_frame, width=30)
        feedback_name.grid(row=0, column=1, pady=5, sticky="w")

        ttk.Label(
            feedback_form_frame, text="Должность:", background="white"
        ).grid(row=1, column=0, pady=5, sticky="e")
        feedback_department = ttk.Entry(feedback_form_frame, width=30)
        ...
        feedback_email = ttk.Entry(feedback_form_frame, width=30)
        ...
        def send_feedback():
            ...
            if not name or not department or not email or not message.strip():
                error_label = ttk.Label(
                    feedback_frame, text="Не удалось отправить сообщение: все поля обязательны для заполнения",
            ...
            if "@" not in email:
                error_label = ttk.Label(
                    feedback_frame, text="Некорректный email: адрес должен содержать знак '@'",
                    foreground="red", background="white"
                )
                error_label.pack(anchor="w", pady=10)
                self.after(3000, error_label.destroy)
